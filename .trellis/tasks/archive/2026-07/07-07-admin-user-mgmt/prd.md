...
- [x] 封禁一个测试用户后，该用户携旧 token 访问任意需登录接口返回 403；启用后恢复正常。（pytest + 浏览器实测）
- [x] 管理员无法封禁/删除自己（后端 400 pytest，前端 admin 行按钮 disabled 实测）。
- [x] keyword/role/status 筛选组合查询结果正确，分页正常。（pytest 组合用例 + UI 搜索实测）
- [x] 删除用户后列表刷新、总数减一，再次登录该账号提示凭证无效。（pytest + 浏览器二次确认删除实测）
- [x] 后端新增端点有对应 pytest 用例（成功 + 自我操作被拒 + 非 admin 403），12 个用例全过，全量 171 passed。
