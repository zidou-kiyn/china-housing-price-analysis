...
- **白名单落地**：`TRAINING_SOURCES=("creprice",)` + `training_rows_only`，加在
  装载入口（predictions._load_source_rows / data_quality._compute_data_fingerprint），
  构建器纯函数与其多源单测零改动、保留路径可逆。
- **破坏性清理已执行**（本地状态，gitignore，不可逆）：
  - `prediction` 表清空（原 6 行 random_forest v1.8 → 0）。
  - 删除 `backend/models/` 下全部模型（random_forest v1.7/v1.8、xgboost v1.0-1.2）
    + `active.json`；保留空 `models/` 目录。
  - 删除遗留备份 `backend/models.bak-governance/`（rf v1.0-1.6、xgboost v1.0-1.1）。
- **空窗态运行时验证**：predict → 404 `NO_ACTIVE_MODEL`；data-quality report →
  200 且 `model_freshness.status=="unknown"`（不 500）；PredictView 显式
  「预测功能数据积累中，暂无可用模型」空态。
- **R5 v1.8 重训前置（留档不执行）**：触发信号 = `07-07-full-data-crawl` 全量首采
  完成（数百城 creprice 覆盖）。届时新开重训任务：白名单生效下训练 v1.8、按模型
  治理流程激活（`PUT /admin/predict/models/active`）。本任务不含重训。
