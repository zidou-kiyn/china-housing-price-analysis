...
- [x] 发起训练接口立即返回 job id，训练在后台执行（期间其他 API 正常响应），任务状态从 running 到 success 可通过轮询观察。
- [x] 训练完成后版本表出现新版本（版本号自增、含指标），活跃指针不变；手动切换活跃后 `GET /predict/{region_id}` 使用新模型（meta 中模型名可验证）。
- [x] 训练进行中再次提交训练返回 409，前端按钮禁用。
- [x] 模型管理页在浏览器中完整走通：查看列表 → 训练 → 进度 → 刷新 → 切换活跃。
- [x] 新增/改动端点有 pytest 用例（train_model 打桩，验证任务流转与互斥；切换活跃持久化）。
