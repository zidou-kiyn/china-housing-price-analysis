...
### Phase 5：短响应污染 bug 修复（2026-07-09）

- [x] 5.1 主会话复核数据发现 368 城市中 120 个 `districts` 为空数组（含广州、郑州、南京、武汉、天津、石家庄、东莞等大城市）。根因：反爬触发时城市详情页偶发返回 HTTP 200 + 约 1432 字节的兜底短页面，`_parse_city_districts()` 从中解析出 0 区县并被当作"确无区县"写入 seed；`should_scrape()` 按当月 `scraped_at` 判断跳过，污染静默保留、续爬不会重爬。
- [x] 5.2 修复：`AsyncHttpClient.get_text()` 新增 `min_length: int = 0` 参数，正文长度不足时抛 `_RetryableError` 复用现有指数退避重试（不影响 `get_json()` 及两个 JSON API）；`Orchestrator._collect_city()` 抓城市详情页时传 `min_length=10000`（正常页最小约 69000 字节、兜底短页约 1432 字节，阈值取两者之间）。`max_retries=3` 与退避逻辑不变。
- [x] 5.3 `uv run ruff check scripts` 通过（`app/api/v1/predictions.py` 有一处与本次改动无关的历史 F401，未处理）。
- [x] 5.4 主会话用真实代理重爬受污染的 120 个城市（分 3 轮：120→55→0 失败收敛），验证通过：368 个城市全部有区县数据，0 篇 0 区县；总区县数从修复前的 2291 增至 3304（找回 1013 个此前被污染丢失的区县及其区县级价格数据）；全部 368 个 JSON 文件结构校验通过。
