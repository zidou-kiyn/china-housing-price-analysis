...
`zidou-kiyn` just ran `trellis init` on a fresh clone, saw "Developer
...
- `.trellis/workspace/zidou-kiyn/journal-*.md` — their session log,
...
- Check if `.trellis/workspace/zidou-kiyn/` already exists — if yes, it's
  their journal from another machine and worth mentioning.
- Run `python3 ./.trellis/scripts/task.py list --assignee zidou-kiyn` to
...
python3 ./.trellis/scripts/task.py archive 00-join-zidou-kiyn
```
