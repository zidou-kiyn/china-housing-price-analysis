...
> 基于机器学习的中国城市房价数据采集、可视化分析与趋势预测系统，覆盖全国 368 城市
...
│   │   └── services/        # 业务逻辑层
│   ├── alembic/             # 数据库迁移
│   ├── scripts/             # 运维脚本（创建管理员、获取 GeoJSON、代理 IP 池批量爬虫等）
│   ├── seed/                # 种子数据（cities.json 城市列表 + prices/ 各城市历史房价）
│   └── tests/               # 后端测试
├── frontend/
│   └── src/
│       ├── api/             # Axios API 模块
