...
class TestCompare:
    async def test_two_regions(self, client, qz_district_ids, auth_headers):
        ids = ",".join(map(str, qz_district_ids[:2]))
        resp = await client.get(
            f"/api/v1/compare?region_type=district&region_ids={ids}", headers=auth_headers
        )
        assert resp.status_code == 200
        data = resp.json()
    ...
    async def test_months_filter(self, client, qz_district_ids, auth_headers):
        ids = ",".join(map(str, qz_district_ids[:2]))
        resp = await client.get(
            f"/api/v1/compare?region_type=district&region_ids={ids}&months=3", headers=auth_headers
        )
        for region in resp.json()["regions"]:
            assert len(region["data"]) <= 3

    async def test_too_few_ids(self, client, qz_district_ids, auth_headers):
        resp = await client.get(
            f"/api/v1/compare?region_type=district&region_ids={qz_district_ids[0]}",
            headers=auth_headers,
        )
        assert resp.status_code == 422

    async def test_too_many_ids(self, client, auth_headers):
        resp = await client.get(
            "/api/v1/compare?region_type=district&region_ids=1,2,3,4,5,6", headers=auth_headers
        )
        assert resp.status_code == 422

    async def test_non_integer_ids(self, client, auth_headers):
        resp = await client.get(
            "/api/v1/compare?region_type=district&region_ids=a,b", headers=auth_headers
        )
        assert resp.status_code == 422

    async def test_404_nonexistent_region(self, client, auth_headers):
        resp = await client.get(
            "/api/v1/compare?region_type=district&region_ids=999998,999999", headers=auth_headers
        )
        assert resp.status_code == 404


class TestMapHeat:
    async def test_returns_all_districts(self, client, qz_district_ids, auth_headers):
        resp = await client.get("/api/v1/map/heat?city_code=qz", headers=auth_headers)
        assert resp.status_code == 200
        data = resp.json()
    ...
    async def test_heat_item_shape(self, client, auth_headers):
        resp = await client.get("/api/v1/map/heat?city_code=qz", headers=auth_headers)
        item = resp.json()["data"][0]
        for key in ("region_id", "region_name", "price"):
            assert key in item

    async def test_404_nonexistent_city(self, client, auth_headers):
        resp = await client.get("/api/v1/map/heat?city_code=nonexistent", headers=auth_headers)
        assert resp.status_code == 404
